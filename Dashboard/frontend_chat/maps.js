...
function addMarker(placeData) {
    ...
    if (displayedPlaceIds.has(placeData.placeId)) {
        return;
    }

    // Get the icon ID from backend (0-7), default to 7 if not provided
    const iconId = placeData.iconId !== undefined ? placeData.iconId : 7;
    ...
    const infoWindowContent = `
        <div style="padding: 10px; max-width: 250px;">
            <h3 style="margin: 0 0 8px 0; font-size: 16px;">${placeData.displayName}</h3>
            ${placeData.formattedAddress ? `<p style="margin: 0 0 8px 0; font-size: 13px; color: #666;">${placeData.formattedAddress}</p>` : ''}
            ${placeData.rating ? `<p style="margin: 0 0 8px 0; font-size: 13px;">⭐ ${placeData.rating} (${placeData.userRatingCount || 0} reviews)</p>` : ''}
            <button onclick="addToSelected('${placeData.placeId}')"
    ...
    displayedPlaceIds.add(placeData.placeId);
...
function panToMarker(placeId) {
    const markerObj = markers.find(m => m.placeData.placeId === placeId);
    if (markerObj) {
