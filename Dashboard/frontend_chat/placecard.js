...
function _showPlaceCardImmediate(placeData, anchorElement) {
    ...
    placeCard.dataset.placeId = placeData.placeId;
    ...
    const googleMapsUrl = placeData.googleMapsUri ||
                         `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(placeData.displayName)}&query_place_id=${placeData.placeId}`;
    ...
    placeCard.innerHTML = `
        ...
        <div class="place-info">
            <h3><a href="${googleMapsUrl}" target="_blank" rel="noopener noreferrer" class="place-name-link">${placeData.displayName}</a></h3>
            ${placeData.rating ? `<p class="rating">⭐ ${placeData.rating} <span class="review-count">(${placeData.userRatingCount || 0})</span></p>` : ''}
            ${placeData.primaryTypeDisplayName ? `<p class="primary-type">${placeData.primaryTypeDisplayName}${placeData.priceLevel ? ' • ' + getPriceDisplay(placeData.priceLevel) : ''}</p>` : ''}
            ${getStatusDisplay(placeData.currentOpeningHours, placeData.businessStatus)}
            <button onclick="addToSelected('${placeData.placeId}'); event.stopPropagation();">+ Add to Route</button>
        </div>
    `;
    ...
    loadPlacePhoto(placeData.placeId, placeCard);
}
