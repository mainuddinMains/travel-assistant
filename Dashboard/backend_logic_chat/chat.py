...
2. grounding_agent: Google Maps grounding agent that enriches places
   ...
   - Extracts placeId from grounding_chunks
   - Returns: Enriched places with full details (rating, hours, placeId, etc.)
...
class TripContext(BaseModel):
    ...
    scheduleStartDate: Optional[str] = None  # Start date in ISO format YYYY-MM-DD (e.g., "2024-12-20")
    scheduleEndDate: Optional[str] = None  # End date in ISO format YYYY-MM-DD (e.g., "2024-12-22")
...
1. **trip_context**: Extract and maintain cities and schedule dates from the ENTIRE conversation history
   ...
   **IMPORTANT: This is stateful - always reflect the CURRENT state of the trip based on ALL messages:**
   ...
   - **scheduleStartDate** and **scheduleEndDate**: Trip dates as ISO strings YYYY-MM-DD (e.g., "2024-12-20", "2024-12-22")
     ...
     * scheduleEndDate must be same as or after scheduleStartDate
...
async def run_cli_chat_async(client: genai.Client, gmap_api_key: str = None):
    ...
    while True:
        ...
        if trip_context:
            ...
            start_date = trip_context.get("scheduleStartDate")
            end_date = trip_context.get("scheduleEndDate")
        ...
        if enrichment_tasks:
            ...
            for i, enriched_place in enumerate(enriched_places):
                if isinstance(enriched_place, Exception):
                    ...
                    all_places.append({"error": str(enriched_place)})
                elif "error" not in enriched_place:
                    all_places.append(enriched_place)
                    # Use originalDisplayName (clean string) for display
                    display_name = enriched_place.get('originalDisplayName', 'Unknown')
                    print(f"\n  ✅ Enriched: {display_name}")
                    print(f"     Place ID: {enriched_place.get('placeId')}")
                    print(f"     Rating: {enriched_place.get('rating')} ({enriched_place.get('userRatingCount')} reviews)")
                else:
                    print(f"\n  ❌ Error: {enriched_place.get('error')}")
                    all_places.append(enriched_place)
    ...
    if all_places:
        ...
        for i, place in enumerate(all_places, 1):
            # Use originalDisplayName and originalFormattedAddress for display
            display_name = place.get('originalDisplayName', 'Unknown')
            print(f"\n{i}. {display_name}")

            # Address
            formatted_address = place.get('originalFormattedAddress')
            if formatted_address:
                print(f"   Address: {formatted_address}")

            # Place ID
            if place.get('placeId'):
                print(f"   Place ID: {place['placeId']}")
