...
def enrich_place_with_details(
    ...
    and returns the details dict with the original place info added.
    ...
    Returns:
        Dict: Google Maps details dict with added metadata fields:
            - All fields from place_details_enterprise() including:
                - displayName (dict): {"text": "...", "languageCode": "..."}
            ...
            - Added metadata fields:
                - "_displayName" (str): Original place name (clean string)
                - "_formattedAddress" (str): Original address (clean string)
                - "_id" (str): Google Maps Place ID
                - "_icon_id" (int): Icon category ID (0-7) for map markers

        Returns place dict with "_error" field if enrichment fails
    ...
    Example:
        ...
        print(enriched["_id"])  # "ChIJ..."
        print(enriched["_displayName"])  # "La Casa Gelato" (clean string)
        print(enriched["displayName"]["text"])  # "La Casa Gelato" (from API)
    ...
    try:
        ...
        if not display_name or not formatted_address:
            place["_error"] = "Missing displayName or formattedAddress in place dict"
            return place
        ...
        if not place_id:
            place["_error"] = f"Place not found for query: {query}"
            return place

        # Step 2: Get full details (Enterprise tier - $25/1000)
        details = place_details_enterprise(
            api_key=api_key,
            place_id=place_id,
            language_code=language_code,
            timeout=timeout,
        ...
        # Step 3: Add all items to the detail dictionary
        details["_displayName"] = place["displayName"]
        details["_formattedAddress"] = place["formattedAddress"]
        details["_id"] = place_id
        # Add icon ID based on primaryType (0-7 for marker icons)
        details["_icon_id"] = get_icon_id(details.get("primaryType"))
