...
function showPlaceWindow(placeData) {
    ...
    placeWindow.innerHTML = `
        ...
        <div class="place-window-content">
            ...
            <div class="place-window-details">
                <p class="address">${placeData.formatted_address || placeData.address || ''}</p>
                ${placeData.rating ? `<p class="rating">⭐ ${placeData.rating} <span class="review-count">(${placeData.user_rating_count || 0} reviews)</span></p>` : ''}
                ${placeData.primary_type ? `<p class="type">${placeData.primary_type}</p>` : ''}
                ${placeData.price_level ? `<p class="price">${getPriceLevelDisplay(placeData.price_level)}</p>` : ''}
                ${getBusinessStatusDisplay(placeData.current_opening_hours, placeData.business_status)}
            </div>
            <div class="place-window-actions">
                <button class="add-to-route-btn" onclick="addToSelected('${placeData.place_id}'); event.stopPropagation();">+ Add to Route</button>
                ${placeData.google_maps_uri ? `<a href="${placeData.google_maps_uri}" target="_blank" class="view-on-maps-btn">View on Google Maps</a>` : ''}
            </div>
        </div>
    `;
    ...
    loadPlaceWindowPhoto(placeData.place_id, placeWindow);
