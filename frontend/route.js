...
async function addToSelected(placeId) {
    ...
    const markerData = markers.find(m => m.placeData.place_id === placeId);
    if (!markerData) return;

    const placeData = markerData.placeData;

    // Check if already selected
    if (selectedPlaces.some(p => p.place_id === placeId)) {
...
function renderSelectedPlaces() {
    ...
    container.innerHTML = selectedPlaces.map((place, index) => {
        ...
        return `
            <div class="place-item" data-index="${index}" data-place-id="${place.place_id}" draggable="true">
                <div class="place-number ${colorClass}">${label}</div>
                <div class="place-info">
                    <div class="place-name">${place.name}</div>
                </div>
                <button class="place-remove" onclick="removeFromSelected('${place.place_id}'); event.stopPropagation();" title="Remove">
                    ✕
                </button>
...
function updateRouteMarkerIcons() {
    // Create a map of place_id to route position
    ...
    selectedPlaces.forEach((place, index) => {
        ...
        routePositions.set(place.place_id, label);
    });

    // Update all markers
    markers.forEach(markerObj => {
        const placeId = markerObj.placeData.place_id;
        ...
        if (routeLabel) {
            ...
            const iconId = markerObj.placeData.icon_id !== undefined ? markerObj.placeData.icon_id : 7;
...
function renderRouteDetails(route) {
    ...
    route.legs.forEach((leg, legIndex) => {
        ...
        const startName = leg.start_name || places[legIndex]?.name || `Stop ${legIndex + 1}`;
    ...
    const endName = lastLeg.end_name || places[places.length - 1]?.name || `Stop ${route.legs.length + 1}`;
    html += `
        <div class="route-place-stop">
 ...
 * @param {Array} places - Array of place objects with place_id, name, formatted_address
...
function convertRouteResponse(result, mode) {
    ...
    const legs = result.legs.map((leg, index) => {
        const legData = {
            ...
            start_name: places[index]?.name || `Stop ${index + 1}`,
            end_name: places[index + 1]?.name || `Stop ${index + 2}`
        };
    ...
    return {
        mode: mode,
        places: places,
        optimized_order: places.map(p => p.place_id),
    ...
    try {
        ...
        if (route.places && route.places.length > 0) {
            ...
            const reorderedPlaces = route.places.map(p => {
                // Find the original place data by place_id
                const original = selectedPlaces.find(sp => sp.place_id === p.place_id);
                return original || p;
            }).filter(Boolean);
